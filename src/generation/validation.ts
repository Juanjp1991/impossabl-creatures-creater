...
import { densityBandForArtworkStage } from "./silhouettePolicy";
  ...
  for (const part of PART_TYPES) {
    ...
    if (draft.layoutMetadata) {
      ...
      const [minElements, maxElements] = draft.layoutMetadata.detailLevel
        ? densityBandForArtworkStage(draft.layoutMetadata.detailLevel, part, draft.layoutMetadata.artworkStage)
        : DENSITY_BANDS[part];
      const densityName = draft.layoutMetadata.artworkStage ?? draft.layoutMetadata.detailLevel ?? "shared";
